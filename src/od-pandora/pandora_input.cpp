...
static int init_mouse(void) { return 1; }

static void close_mouse(void) {}

static int acquire_mouse(int num, int flags) { return 1; }

static void unacquire_mouse(int num) {}

static int get_mouse_num(void) { return 2; }
...
static void read_mouse(void) {
  ...
  if (currprefs.jports[0].id == JSEM_MICE + 1 ||
      currprefs.jports[1].id == JSEM_MICE + 1) {
    // dPad is mouse
...
static int get_mouse_flags(int num) { return 0; }

struct inputdevice_functions inputdevicefunc_mouse = {init_mouse,
                                                      close_mouse,
                                                      acquire_mouse,
                                                      unacquire_mouse,
                                                      read_mouse,
...
static void setid(struct uae_input_device *uid, int i, int slot, int sub,
                  int port, int evt, bool gp) {
  if (gp)
...
static void setid_af(struct uae_input_device *uid, int i, int slot, int sub,
                     int port, int evt, int af, bool gp) {
...
int input_get_default_mouse(struct uae_input_device *uid, int i, int port,
                            int af, bool gp, bool wheel, bool joymouseswap) {
  setid(uid, i, ID_AXIS_OFFSET + 0, 0, port,
        port ? INPUTEVENT_MOUSE2_HORIZ : INPUTEVENT_MOUSE1_HORIZ, gp);
  setid(uid, i, ID_AXIS_OFFSET + 1, 0, port,
        port ? INPUTEVENT_MOUSE2_VERT : INPUTEVENT_MOUSE1_VERT, gp);
  setid_af(uid, i, ID_BUTTON_OFFSET + 0, 0, port,
           port ? INPUTEVENT_JOY2_FIRE_BUTTON : INPUTEVENT_JOY1_FIRE_BUTTON, af,
           gp);
...
static int init_kb(void) { return 1; }

static void close_kb(void) {}

static int acquire_kb(int num, int flags) { return 1; }

static void unacquire_kb(int num) {}

static void read_kb(void) {}

static int get_kb_num(void) { return 1; }

static TCHAR *get_kb_friendlyname(int kb) { return strdup("Default Keyboard"); }

static TCHAR *get_kb_uniquename(int kb) { return strdup("KEYBOARD0"); }

static int get_kb_widget_num(int kb) { return 255; }

static int get_kb_widget_first(int kb, int type) { return 0; }
...
static int get_kb_flags(int num) { return 0; }

struct inputdevice_functions inputdevicefunc_keyboard = {init_kb,
                                                         close_kb,
                                                         acquire_kb,
                                                         unacquire_kb,
                                                         read_kb,
...
static int init_joystick(void) {
  // This function is called too many times... we can filter if number of joy is
  // good...
...
static int acquire_joystick(int num, int flags) { return 1; }
...
static int get_joystick_widget_type(int joy, int num, TCHAR *name,
                                    uae_u32 *code) {
...
static int get_joystick_flags(int num) { return 0; }
...
void handle_remapped_press(const int hostjoyid, const int input_key, handled_keys& handled_keys) {
  const int target = customControlMap[input_key];
  write_log(_T("Source: %i target %i\n"), input_key, target);
  switch (target) {
    ...
    case REMAP_JOYBUTTON_ONE:
      //setjoybuttonstate(hostjoyid + 1, 0, 1);
      handled_keys.button_1 = true;
      break;
    case REMAP_JOYBUTTON_TWO:
      //setjoybuttonstate(hostjoyid + 1, 1, 1);
      handled_keys.button_2 = true;
      break;
    case REMAP_JOY_UP:
      setjoystickstate(hostjoyid + 1, 1, -32767, 32767);
      handled_keys.y_axis = true;
      break;
    case REMAP_JOY_DOWN:
      setjoystickstate(hostjoyid + 1, 1, 32767, 32767);
      handled_keys.y_axis = true;
    ...
    case REMAP_CD32_GREEN:
      //setjoybuttonstate(hostjoyid + 1, 2, 1);
      handled_keys.button_3 = true;
      break;
    case REMAP_CD32_YELLOW:
      //setjoybuttonstate(hostjoyid + 1, 3, 1);
      handled_keys.button_4 = true;
      break;
...
static void read_joystick(void) {
  ...
  for (int joyid = 0; joyid < MAX_JPORTS; joyid++)
    ...
    if (currprefs.jports[joyid].id == JSEM_JOYS) {
      ...
      if (!keystate[VK_R]) { // Right shoulder + dPad -> cursor keys
        int axis =
            (keystate[VK_LEFT] ? -32767 : (keystate[VK_RIGHT] ? 32767 : 0));
        if (!joyXviaCustom)
      ...
      int val_x = SDL_JoystickGetAxis(Joysticktable[hostjoyid], 0);
      int val_y = SDL_JoystickGetAxis(Joysticktable[hostjoyid], 1);
      handled_keys keys;
      memset(&keys, 0, sizeof(handled_keys));
      if (currprefs.pandora_customControls) {
        if (hat & SDL_HAT_RIGHT)
          handle_remapped_press(hostjoyid, VK_RIGHT, keys);
        else if (hat & SDL_HAT_LEFT)
          handle_remapped_press(hostjoyid, VK_LEFT, keys);
        if (hat & SDL_HAT_UP)
          handle_remapped_press(hostjoyid, VK_UP, keys);
        else if (hat & SDL_HAT_DOWN)
          handle_remapped_press(hostjoyid, VK_DOWN, keys);

        // Square
        if (SDL_JoystickGetButton(Joysticktable[hostjoyid], 0) & 1) {
          handle_remapped_press(hostjoyid, VK_A, keys);
        }
        // X
        if (SDL_JoystickGetButton(Joysticktable[hostjoyid], 1) & 1) {
          handle_remapped_press(hostjoyid, VK_B, keys);
        }
        // Circle
        if (SDL_JoystickGetButton(Joysticktable[hostjoyid], 2) & 1) {
          handle_remapped_press(hostjoyid, VK_X, keys);
        }
        // Triangle
        ...
        if (!keys.y_axis) {
          setjoystickstate(hostjoyid + 1, 1, val_y, 32767);
        }
        setjoybuttonstate(hostjoyid + 1, 0, keys.button_1);
        ...
        /*if (IsPS3Controller[hostjoyid]) {
          setjoybuttonstate(
              hostjoyid + 1, 0,
              (SDL_JoystickGetButton(Joysticktable[hostjoyid], 13) & 1));
          setjoybuttonstate(
              hostjoyid + 1, 1,
              (SDL_JoystickGetButton(Joysticktable[hostjoyid], 14) & 1));
        ...
        setjoybuttonstate(
            hostjoyid + 1, 0,
            (SDL_JoystickGetButton(Joysticktable[hostjoyid], 0) & 1));
        setjoybuttonstate(
            hostjoyid + 1, 1,
            (SDL_JoystickGetButton(Joysticktable[hostjoyid], 1) & 1));
        setjoybuttonstate(
            hostjoyid + 1, 2,
            (SDL_JoystickGetButton(Joysticktable[hostjoyid], 2) & 1));
        setjoybuttonstate(
            hostjoyid + 1, 3,
            (SDL_JoystickGetButton(Joysticktable[hostjoyid], 3) & 1));
...
int input_get_default_joystick(struct uae_input_device *uid, int num, int port,
                               int af, int mode, bool gp, bool joymouseswap) {
  int h, v;
...
int input_get_default_joystick_analog(struct uae_input_device *uid, int num,
                                      int port, int af, bool joymouseswap) {
  return 0;
}
